...
**Do not file security issues as public GitHub issues, Discord posts, or PRs.** A public report is itself an exploit broadcast — many downstream projects pull BeraBaileys directly from `master` or from `github:bera-team/bera-baileys`, so a vulnerable version is in the wild within minutes.

Use either of these private channels:

1. **Preferred:** [GitHub Security Advisories](https://github.com/bera-team/bera-baileys/security/advisories/new) on this repository. This gives us a private workspace to coordinate the fix, draft a CVE, and notify downstream projects.
...
- Vulnerabilities in dependencies that are reachable through BeraBaileys' public API
...
- Bugs that only affect availability of the local process (a malformed input that throws an exception in your handler is not a CVE; a malformed input that lets a remote party crash any BeraBaileys client is)
- Issues in third-party WhatsApp clones or forks of BeraBaileys
- Behavior of WhatsApp itself — we don't control the server; report those to Meta
- Missing rate limiting in *your* application — BeraBaileys doesn't add rate limiting on top of WhatsApp's
...
A few notes for downstream users — these aren't vulnerabilities in BeraBaileys, but they are common ways production systems leak:

- **`bera-baileys_auth_info/` is equivalent to a long-lived credential.** Store it like an SSH private key. Don't commit it. Don't put it in container images. Encrypt it at rest if you can.
...
- **Pin a release.** Pulling `github:bera-team/bera-baileys` always grabs `master`. Use a tagged release in production.

For non-security questions, see Discord (https://discord.gg/WeJM5FP9GG) or the wiki (https://bera-baileys.wiki).
