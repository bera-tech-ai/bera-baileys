...
Guide for AI coding agents (Claude Code, Cursor, Aider, Codex, Copilot Workspace, etc.) contributing to BeraBaileys. Human contributors should also read this — there's nothing AI-specific in the conventions, only in the disclosure rules at the end.
...
src/
  ...
  Defaults/        Constants (WA Web version, bera-baileys version, default config)
...
3. Don't commit `bera-baileys_auth_info/`, `.env`, `mitm_*.db`, `.superset/`, or any session state. Git is configured to ignore the obvious ones; double-check.
...
- **`Defaults/bera-baileys-version.json`** — bumped by the `update-version` workflow. Manual edits race with automation.
...
- **Don't paste auth state into AI tools.** `bera-baileys_auth_info/` contains long-lived Signal keys. Treat it like an SSH private key.
