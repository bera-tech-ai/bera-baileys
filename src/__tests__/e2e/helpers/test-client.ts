...
const uniquePushName = (prefix = 'bera-baileys-e2e') => `${prefix}-${randomUUID()}`
...
const resolveConfig = (opts: TestClientOptions): ResolvedConfig => ({
	...
	authDir: opts.authDir ?? join(tmpdir(), `bera-baileys-e2e-${randomUUID()}`),
...
const openConnection = async (
	...
	for (;;) {
		...
		// pairing-handoff close: BeraBaileys terminates the noise socket once paired, we re-open
		config.logger.debug({ reason: result.reason }, 'reconnecting after pairing handoff')
	}
}

/**
 * BeraBaileys socket wrapped with e2e ergonomics: QR autoscan via bartender admin,
...
export class TestClient {
	...
	async resolveTestGroup(name = 'BeraBaileys Group Test'): Promise<string | undefined> {
	...
	waitForEvent<K extends keyof BeraBaileysEventMap>(
		event: K,
		predicate: (data: BeraBaileysEventMap[K]) => boolean,
		timeoutMs = DEFAULT_TIMEOUT_MS
	): Promise<BeraBaileysEventMap[K]> {
		return waitWithTimeout<BeraBaileysEventMap[K]>(
			emit => {
				const handler = (data: BeraBaileysEventMap[K]) => {
