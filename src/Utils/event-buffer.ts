 ...
 */
type BeraBaileysEventData = Partial<BeraBaileysEventMap>

const BUFFERABLE_EVENT_SET = new Set<BeraBaileysEvent>(BUFFERABLE_EVENT)

type BeraBaileysBufferableEventEmitter = BeraBaileysEventEmitter & {
	/** Use to process events in a batch */
	process(handler: (events: BeraBaileysEventData) => void | Promise<void>): () => void
	/**
 ...
 */
export const makeEventBuffer = (logger: ILogger): BeraBaileysBufferableEventEmitter => {
	...
	// take the generic event and fire it as a bera-baileys event
	ev.on('event', (map: BeraBaileysEventData) => {
		for (const event in map) {
			ev.emit(event, map[event as keyof BeraBaileysEventMap])
		}
	})
	...
	return {
		process(handler) {
			const listener = async (map: BeraBaileysEventData) => {
				await handler(map)
			}
		...
		emit<T extends BeraBaileysEvent>(event: BeraBaileysEvent, evData: BeraBaileysEventMap[T]) {
			...
			if (event === 'messages.upsert') {
				const { type } = evData as BeraBaileysEventMap['messages.upsert']
...
function append<E extends BufferableEvent>(
	...
	switch (event) {
		...
		case 'contacts.update':
			const contactUpdates = eventData as BeraBaileysEventMap['contacts.update']
		...
		case 'messages.upsert':
			const { messages, type } = eventData as BeraBaileysEventMap['messages.upsert']
		...
		case 'messages.update':
			const msgUpdates = eventData as BeraBaileysEventMap['messages.update']
		...
		case 'messages.delete':
			const deleteData = eventData as BeraBaileysEventMap['messages.delete']
		...
		case 'messages.reaction':
			const reactions = eventData as BeraBaileysEventMap['messages.reaction']
		...
		case 'message-receipt.update':
			const receipts = eventData as BeraBaileysEventMap['message-receipt.update']
			...
			break
		case 'groups.update':
			const groupUpdates = eventData as BeraBaileysEventMap['groups.update']
...
function consolidateEvents(data: BufferedEventData) {
	const map: BeraBaileysEventData = {}
