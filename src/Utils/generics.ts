...
const beraBaileysVersion = [2, 3000, 1035194821]
import type {
...
export function bindWaitForEvent<T extends keyof BeraBaileysEventMap>(ev: BeraBaileysEventEmitter, event: T) {
	return async (check: (u: BeraBaileysEventMap[T]) => Promise<boolean | undefined>, timeoutMs?: number) => {
		let listener: (item: BeraBaileysEventMap[T]) => void
...
export const bindWaitForConnectionUpdate = (ev: BeraBaileysEventEmitter) => bindWaitForEvent(ev, 'connection.update')

/**
 * utility that fetches latest bera-baileys version from the master branch.
 * Use to ensure your WA connection is always on the latest version
 */
export const fetchLatestBeraBaileysVersion = async (options: RequestInit = {}) => {
	const URL = 'https://raw.githubusercontent.com/bera-team/bera-baileys/master/src/Defaults/index.ts'
	try {
		...
		if (!response.ok) {
			throw new Boom(`Failed to fetch latest BeraBaileys version: ${response.statusText}`, { statusCode: response.status })
		}
		...
		return {
			version: beraBaileysVersion as WAVersion,
			isLatest: false,
			error
		}
	}
...
/**
 * Stream errors generally provide a reason, map that to a bera-baileys DisconnectReason
