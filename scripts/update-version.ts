 ...
 * - src/Defaults/bera-baileys-version.json
...
function updateBeraBaileysVersionJson(version: [number, number, number]): boolean {
	const filePath = join(ROOT_DIR, 'src/Defaults/bera-baileys-version.json')
	const content = {
		version
	}
	...
	try {
		...
		if (currentVersion[0] === version[0] && currentVersion[1] === version[1] && currentVersion[2] === version[2]) {
			console.log(`✓ bera-baileys-version.json already up to date`)
			return false
		}

		writeFileSync(filePath, JSON.stringify(content) + '\n')
		console.log(`✓ Updated bera-baileys-version.json: [${currentVersion.join(', ')}] → [${version.join(', ')}]`)
		return true
	} catch (error) {
		console.error(`✗ Failed to update bera-baileys-version.json:`, error)
		throw error
	}
}
...
function updateGenerics(version: [number, number, number]): boolean {
	...
	try {
		...
		const versionRegex = /const bera-baileysVersion = \[(\d+),\s*(\d+),\s*(\d+)\]/
		const match = content.match(versionRegex)

		if (!match) {
			throw new Error('Could not find bera-baileysVersion declaration in generics.ts')
		}
		...
		const newContent = content.replace(
			versionRegex,
			`const bera-baileysVersion = [${version[0]}, ${version[1]}, ${version[2]}]`
		)
...
async function main() {
	...
	const updates = [
		updateBeraBaileysVersionJson(result.version),
